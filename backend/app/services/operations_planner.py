    ...
    def _build_fr24_params(operation: Operation, plan) -> dict:
        ...
        if cap == "live_positions":
            ...
            if operation.scope_entity_id and operation.scope_entity_type == "airline":
                params["operating_as"] = operation.scope_entity_id  # Truth: OperatingAs shared param
            if operation.scope_filters:
                if "operator_icao" in operation.scope_filters:
                    params["operating_as"] = operation.scope_filters["operator_icao"]  # Truth: OperatingAs param
                if "airports" in operation.scope_filters:
                    params["airports"] = operation.scope_filters["airports"]  # Truth: Airports shared param
                if "painted_as" in operation.scope_filters:
                    params["painted_as"] = operation.scope_filters["painted_as"]  # Truth: PaintedAs param
                if "callsigns" in operation.scope_filters:
                    params["callsigns"] = operation.scope_filters["callsigns"]  # Truth: Callsigns param
                if "registrations" in operation.scope_filters:
                    params["registrations"] = operation.scope_filters["registrations"]  # Truth: Registrations param
            params["limit"] = 100
