...
def _dispatch_operation_task(operation_id: int) -> None:
    """
    Dispatches the Celery execution task.
    FIX BUG-2: Connection refused when backend can't reach Redis broker.
    
    Strategy:
      1. Try Celery dispatch (primary).
      2. If broker unreachable → mark operation with failure_reason so
         the worker's retry_chunks_task can pick it up on next tick,
         OR the user can re-approve.
    
    Root cause in logs: ops 77-83 all got [Errno 111] Connection refused
    because the backend container couldn't reach the Redis broker at
    dispatch time (broker URL may have been localhost instead of service name).
    """
    import logging
    log = logging.getLogger(__name__)
    try:
        from worker.tasks.operations_task import execute_operation_task
        result = execute_operation_task.delay(operation_id)
        log.info(f"[Operations] Task dispatched for op {operation_id}: {result.id}")
    except Exception as exc:
        log.error(
            f"[Operations] Failed to dispatch task for op {operation_id}: {exc}. "
            f"Operation remains in 'running' state — worker retry_chunks_task "
            f"will pick up pending chunks on next 30s tick."
        )
        # Mark the failure reason so the ops board shows the issue
        # but DO NOT change status — the pending chunks will be picked
        # up by retry_chunks_task which polls independently of the broker dispatch
        try:
            ...
            try:
                ...
                if op and not op.failure_reason:
                    op.failure_reason = (
                        f"Worker dispatch failed: {exc}. "
                        "Chunks queued — will execute on next worker poll."
                    )
                    db.commit()
            finally:
                db.close()
        except Exception as db_exc:
            log.error(f"[Operations] Could not update failure_reason: {db_exc}")
