    ...
    def process_telemetry_batch(
        ...
        for payload in payloads:
            try:
                ...
                # Truth: FlightPositionsFull.source + .eta
                current_state.source         = payload.source
                current_state.eta            = payload.eta
...
class AnalyticsCRUD:
    ...
    def get_daily_summary(db: Session, date_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"[AnalyticsCRUD] Invalid date_str '{date_str}', defaulting to today")
...
def _apply_date_filter_session(q, date_from: Optional[str], date_to: Optional[str]):
    ...
    if date_from:
        try:
            ...
            logger.warning(f"[DateFilter] Invalid date_from: {date_from!r}")
    if date_to:
        try:
            ...
            logger.warning(f"[DateFilter] Invalid date_to: {date_to!r}")
    return q
