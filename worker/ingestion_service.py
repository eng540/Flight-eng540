...
class FlightIngestionService:
        ...
        try:
            for i in range(0, len(fr24_ids), chunk_size):
                ...
                data = self._safe_request(
                    ...
                    {"flight_ids": ids_param, "limit": len(chunk)},  # Truth: FlightIdsFlightSummary param = flight_ids
                )
                if not data:
                    errors += len(chunk)
                    continue
    ...
    def fetch_historical_track(self, fr24_id: str) -> Optional[List[Dict]]:
        ...
        # FR24 FlightTracks response is an ARRAY at root level:
        # [{fr24_id: "...", tracks: [{timestamp, lat, lon, ...}]}]
        # Truth: curl shows array: [{"fr24_id":"35dc6f7d","tracks":[...]}]
        # Fix: handle both array (actual) and dict (spec schema) format
        if isinstance(data, list):
            flight_data = data[0] if data else {}
        else:
            flight_data = data
        tracks_raw = flight_data.get("tracks", [])
        if not tracks_raw:
            return []
    ...
    def _parse_fr24_position(
        ...
        return RawIngestionPayload(
            ...
            squawk=f.get("squawk"),
            # Truth: FlightPositionsFull.source — data source (ADSB, MLAT, ESTIMATED, etc.)
            source=f.get("source"),
